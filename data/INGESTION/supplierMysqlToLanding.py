...
spark = SparkSession.builder.appName("RetailerMysqlDBToLanding").getOrCreate()
...
CONFIG_PATH = f"gs://{GCS_BUCKET}/configs/supplier_config.csv"
...
#### jdbc config
database = "supplierDB"
jdbc_url = f"jdbc:mysql://34.122.161.24:3306/{database}" 
user = "harish"
password = "Harish@123"
...
# MySQL Configuration
MYSQL_CONFIG = {
    "url": "jdbc:mysql://34.122.161.24:3306/supplierDB?useSSL=true&allowPublicKeyRetrieval=true",
    "driver": "com.mysql.cj.jdbc.Driver",
    "user": "harish",
    "password": "Harish@123"
}
...
def save_log_to_gcs():
    log_filename = f"pipeline_log_{datetime.now().strftime('%Y%m%d%H%M')}.json"
...
def move_existing_files_to_archive(table):
    blobs = storage_client.bucket(GCS_BUCKET).list_blobs(prefix=f"landing/supplier-db/{table}/")
...
def get_latest_watermark(tablename):
    query = f"""
           select max(load_timestamp) as latest_timestamp 
           from `{BQ_AUDIT_TABLE}` where tablename = '{tablename}'
    """ 
